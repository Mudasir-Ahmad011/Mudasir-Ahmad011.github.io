...
// $php_main_email = "mudasirofficially@gmail.com";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //After sanitization, perform validation
    if (filter_var($php_email, FILTER_VALIDATE_EMAIL)) {
        $php_subject = "Message from contact form";
        ...
        // Send mail by PHP Mail Function
        $mailSent = mail($php_main_email, $php_subject, $php_template, $php_headers);

        if ($mailSent) {
            echo '<div class="returnmessage" data-success="Your message has been received, We will contact you soon."></div>';
        } else {
            echo '<div class="returnmessage" data-error="Oops! Something went wrong, please try again later."></div>';
        }
    } else {
        echo "<span class='contact_error'>* Invalid email *</span>";
    }
